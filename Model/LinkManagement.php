...
class LinkManagement implements \Credevlab\ConfigurableBundle\Api\ProductLinkManagementInterface
{
    ...
    public function saveChild(
        ...
        if ($product->getTypeId() != Type::TYPE_CODE) {
            throw new InputException(
                __('Product with specified sku: "%1" is not a configurable bundle product', [$product->getSku()])
            );
        }
    ...
    public function removeChild($sku, $optionId, $childSku)
    {
        ...
        if ($product->getTypeId() != Type::TYPE_CODE) {
            throw new InputException(__('Product with specified sku: %1 is not a configurable bundle product', $sku));
        }
